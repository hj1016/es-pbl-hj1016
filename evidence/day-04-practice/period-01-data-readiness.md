...
- 제출 상태 확인: 5분
- 시작 기준: Kibana 접속 가능
- 화면 순서: [Data View·Discover 상세 가이드](https://github.com/djkorea/es-5days-pbl-course/blob/main/day-04/KIBANA_9_5_STEP_BY_STEP.md#1-data-view-만들기-또는-기존-data-view-확인하기)
...
- 한 문서가 의미하는 대상 또는 사건: 개발 오류 해결 사례 1건
- Dashboard 사용자: 개발 중 오류 해결 사례를 찾고 현황을 파악하려는 개발자
- 사용자가 내릴 판단: 오류 유형·기술·검증 여부·해결 시간 분포를 확인하고 우선 검토할 사례를 정한다.
- 첫 분석 질문: 전체 오류 해결 사례는 몇 건인가?
- 필요한 field: `case_id`
- 각 field의 mapping type: `case_id`는 `keyword`
- 실제 존재 여부: 존재
- 데이터 문서 수: 50,000건
- A 개인 데이터 사용 / B 공통 products 사용+보강 설계 / C 공통 실습+개인 청사진 중 선택: A 개인 데이터 사용
- 선택 이유: `devfix-cases`에 50,000건이 있고 범주·수치·상태·날짜 field가 모두 있어 개인 Dashboard를 제작할 수 있다.
- 부족한 데이터와 다음 행동: 실제 장애의 심각도와 SLA 준수 여부를 판단하려면 `severity`, `resolved_at`, `sla_met` field가 추가로 필요하다.
- 캡처 파일: `assets/devfix-discover-50000.png`
